...
class ProfileController {
    ...
    async createProfile(req, res, next) {
        try {
            ...
            // Cloudinary tự tạo URL ở req.file.path, còn multer-s3 (R2/AWS) trả về req.file.key (đường dẫn object) & req.file.location (S3 URL).
            // Nếu có S3_PUBLIC_DOMAIN (R2 public domain) thì ưu tiên ghép S3_PUBLIC_DOMAIN/key, ngược lại dùng location làm fallback.
            const avatarUrl = process.env.S3_PUBLIC_DOMAIN ? `${process.env.S3_PUBLIC_DOMAIN.replace(/\/$/, '')}/${req.file.key}` : req.file.location;
    ...
    async getPublicProfile(req, res, next) {
        try {
            ...
            const profile = await getOrSetCache(cacheKey, async () => {
                // .lean() returns a plain JS object instead of a full Mongoose Document for faster read performance
                return await Profile.findOne({ username, isActive: true }).lean();
            }, 600);
            ...
            // max-age: trình duyệt cache trong 600s
            // s-maxage: CDN cache trong 600s
            // stale-while-revalidate: trình duyệt vẫn hiển thị dữ liệu cũ trong 60s khi đang tải dữ liệu mới
        ...
        try {
            ...
            if (req.file) {
                if (currentProfile.avatarUrl) {
                    // await deleteCloudinaryImage(currentProfile.avatarUrl);
                    await deleteR2Image(currentProfile.avatarUrl);
                }
                // currentProfile.avatarUrl = req.file.path;
                currentProfile.avatarUrl = process.env.S3_PUBLIC_DOMAIN ? `${process.env.S3_PUBLIC_DOMAIN.replace(/\/$/, '')}/${req.file.key}` : req.file.location;
            }
            // QUAN TRỌNG: Báo cho Mongoose biết field 'design' đã thay đổi
            currentProfile.markModified('design');
            await currentProfile.save();

            // Invalidate public profile cache for old and new username if changed
            if (oldUsername) delCache(`profile:public:${oldUsername}`);
            delCache(`profile:public:${currentProfile.username}`);
    ...
    async updateDesign(req, res, next) {
        try {
            ...
            // Invalidate public profile cache so new design reflects immediately
            delCache(`profile:public:${currentProfile.username}`);
    ...
    async uploadBackground(req, res, next) {
        try {
            ...
            if (currentProfile.design?.background?.imageUrl) {
                // await deleteCloudinaryImage(currentProfile.design.background.imageUrl);
                await deleteR2Image(currentProfile.design.background.imageUrl);
            }
            ...
            // Invalidate public profile cache when background image changes
            delCache(`profile:public:${currentProfile.username}`);
    ...
    async deactivateProfile(req, res, next) {
        try {
            ...
            // Update isActive to false to deactivate profile and invalidate cache
            const profile = await Profile.findByIdAndUpdate(profileId, { isActive: false }, { new: true });
            if (profile) {
                delCache(`profile:public:${profile.username}`);
            }
    ...
    async reactivateProfile(req, res, next) {
        try {
            const { profileId } = req.params;
            const profile = await Profile.findByIdAndUpdate(profileId, { isActive: true }, { new: true });
            if (profile) {
                delCache(`profile:public:${profile.username}`);
            }
