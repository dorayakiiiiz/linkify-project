...
import { sendSSEEvent } from "../utils/sseManager.mjs";
...
export const checkLinkContent = async (linkId, title, url) => {
  try {
    ...
    const isViolating = result.isViolating && result.confidence > 70;
    ...
    if (isViolating) {
      ...
      if (currentLink.moderationStatus !== 'flagged' && currentLink.profileId) {
    ...
    // Broadcast real-time SSE event to profile client stream
    if (currentLink.profileId) {
      delCache(`links:public:${currentLink.profileId}`);
      sendSSEEvent(currentLink.profileId.toString(), {
        type: 'link',
        id: linkId.toString(),
        moderationStatus: isViolating ? 'flagged' : 'approved',
        isEnable: !isViolating,
        violationReason: isViolating ? result.reason : null,
      });
    }
  } catch (err) {
...
export const checkProductContent = async (productId, name, price, url) => {
  try {
    ...
    if (isViolating) {
      ...
      if (currentProduct.moderationStatus !== 'flagged' && currentProduct.profileId) {
        const profile = await Profile.findById(currentProduct.profileId);
        if (profile && profile.userId) {
          await User.findByIdAndUpdate(profile.userId, {
            $inc: { violationCount: 1 },
          });
    ...
    if (currentProduct.profileId) {
      delCache(`products:public:${currentProduct.profileId}`);
      sendSSEEvent(currentProduct.profileId.toString(), {
        type: 'product',
