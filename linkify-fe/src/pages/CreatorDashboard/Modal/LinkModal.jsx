...
export default function LinkModal({ onClose, editingLink = null }) {
    ...
    const [showSchedule, setShowSchedule] = useState((editingLink?.scheduledEnable || editingLink?.scheduledDisable) ? true : false);
    
    const [title, setTitle] = useState(editingLink?.title || "");
    const [url, setUrl] = useState(editingLink?.url || "");
    const [scheduledEnable, setScheduledEnable] = useState(formatDate(editingLink?.scheduledEnable));
    const [scheduledDisable, setScheduledDisable] = useState(formatDate(editingLink?.scheduledDisable))
    ...
    return (
        ...
        >
            <div 
                className="flex flex-col w-full max-w-[750px] px-[40px] py-[26px] min-h-[460px] bg-[#fff] md:rounded-3xl"
            ...
            >
                ...
                <div className="w-full flex justify-between items-center">
                    <div className="font-momo text-2xl">
                        ...
                        <i className="fa-solid fa-link ml-[10px] text-[#0099ff]"></i>
                    </div>
                    <div 
                ...
                <div className="w-full flex-1 flex flex-col">
                    ...
                    {!editingLink && (
                        <div className="w-full mt-[20px]">
                            ...
                            <div className="text-[#5f6060] font-semibold mb-[6px]">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div 
                                    ref={scrollRef}
                                    className="flex items-center gap-[16px] px-[10px] py-[6px] rounded-xl overflow-x-auto no-scrollbar"
                                >
                                    ...
                                    {SOCIALS.map(platform => {
                                        return (
                                            <div
                                                key={platform.id}
                                                className={`cursor-pointer h-[80px] w-[80px] flex-shrink-0 flex flex-col items-center gap-[4px] justify-center rounded-2xl border ${platform.id === selectedPlatform?.id ? 'border-[#000] border-[2px]' : 'border-[#E0E2D9]'}  hover:translate-y-[-2px] transition`}
                    ...
                    <div className="w-full mt-[20px] flex flex-col md:flex-row gap-[20px]">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center justify-between">
                                <input 
                                    type="text"
                                    className="h-[50px] w-full rounded-xl bg-[#f7f8f6] px-[20px]"
                                    value={title}
                    ...
                    <div className="w-full mt-[20px]">
                        <div
                            className="w-fit flex items-center gap-[10px] cursor-pointer"
                            onClick={() => setShowSchedule(!showSchedule)}
                        >
                            <i className="fa-regular fa-clock"></i>
                            Schedule link (optional)
                            <i className={`fa-solid fa-chevron-down text-xs transition-transform duration-200 ${showSchedule ? 'rotate-180' : ''}`}></i>
                        </div>
