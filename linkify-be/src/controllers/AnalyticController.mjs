...
class AnalyticController {
    ...
    async trackEvent(req, res) {
        try {
            ...
            // 1. Lưu log chi tiết vào bảng Analytic
            ...
            // 2. Tăng counter nhanh trong bảng Link/Product (để sort nhanh ở các chỗ khác)
            if (type === 'link_click' && targetId) {
                await Link.findByIdAndUpdate(targetId, { $inc: { clickCount: 1 } });
            } else if (type === 'shop_click' && targetId) {
                await Product.findByIdAndUpdate(targetId, { $inc: { clickCount: 1 } });
            }
