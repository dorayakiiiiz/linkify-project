    ...
    const columns = [
        ...
        },
        {
            ...
            render: (item) => {
                ...
                if (item.moderationStatus !== 'flagged') return <span className="text-green-600 text-xs font-bold"><i className="fa-solid fa-check mr-1"></i>Safe</span>;
                return (
        ...
        },
        {
            ...
            render: (item) => (
                <div className={`px-2 py-1 rounded-full text-xs font-semibold w-fit ${
                    item.moderationStatus === 'flagged' ? 'bg-red-100 text-red-700' : item.moderationStatus === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'
                }`}>
                    {item.moderationStatus === 'flagged' ? 'Hidden (Flagged)' : item.moderationStatus === 'pending' ? 'Pending' : 'Active'}
                </div>
            )
        }
    ];
