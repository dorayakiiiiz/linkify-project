...
export default function LinkManagementPage() {
    ...
    const columns = [
        ...
        },
        {
            header: "AI Detection",
            render: (link) => {
                if (link.moderationStatus === 'pending') {
                    return <span className="text-amber-600 text-xs font-bold"><i className="fa-solid fa-spinner animate-spin mr-1"></i>Pending Review</span>;
                }
                if (link.moderationStatus !== 'flagged') return <span className="text-green-600 text-xs font-bold"><i className="fa-solid fa-check mr-1"></i>Safe</span>;
                return (
        ...
        },
        {
            ...
            render: (link) => (
                <div className={`w-fit px-2 py-1 rounded-full text-xs font-semibold ${
                    link.moderationStatus === 'flagged' ? 'bg-red-100 text-red-700' : link.moderationStatus === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'
                }`}>
                    {link.moderationStatus === 'flagged' ? 'Hidden' : link.moderationStatus === 'pending' ? 'Pending' : 'Active'}
                </div>
            )
        }
    ];
